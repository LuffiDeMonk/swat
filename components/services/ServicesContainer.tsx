...
export default function ServicesContainer() {
    return (
        <section id='services'>
            <Container>
                <Tabs defaultValue={ServiceTabs[0].title} className="h-full flex flex-col lg:flex-row lg:gap-20">
                    <TabListContainer />
            ...
            <div className="h-[80vh] -mt-[10vh] relative">
                ...
                <MotionDiv variants={variants} initial='initial' whileInView='whileInView' viewport={{ once: true }} className="w-full h-full absolute top-0 py-0.5 right-0 flex flex-col items-end gap-0.5">
                    {
                        Array(4).fill('q').map((_, idx) => (
                            <MotionDiv variants={variants} className="bg-white bg-opacity-90 h-[25%] p-6 hover:-translate-x-5 w-full lg:w-1/2 transition-transform group" key={idx}>
